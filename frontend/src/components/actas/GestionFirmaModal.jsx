/* eslint-disable */
// components/actas/GestionFirmaModal.jsx
// Modal para guardar/reemplazar/eliminar la firma personal del usuario.

import React, { useState, useRef } from 'react';
import { motion } from 'framer-motion';
import { X, Upload, Save } from 'lucide-react';

export default function GestionFirmaModal({ firmaActual, onSave, onDelete, onClose }) {
    const canvasRef = useRef(null);
    const [drawing, setDrawing] = useState(false);
    const [hasDrawn, setHasDrawn] = useState(false);
    const [uploadedUrl, setUploadedUrl] = useState(null);

    const getPos = (e) => { const rect = canvasRef.current.getBoundingClientRect(); const t = e.touches?.[0] || e; return { x: t.clientX - rect.left, y: t.clientY - rect.top }; };
    ...
    const clearCanvas = () => { canvasRef.current.getContext('2d').clearRect(0, 0, canvasRef.current.width, canvasRef.current.height); setHasDrawn(false); setUploadedUrl(null); };
    ...
    const handleSave = () => {
        const data = uploadedUrl || (hasDrawn && canvasRef.current ? canvasRef.current.toDataURL('image/png') : null);
        if (data) onSave(data);
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                <div className="p-5 space-y-4">
                    <p className="text-sm text-slate-500">Tu firma se guardará para firmar actas con un solo clic.</p>
